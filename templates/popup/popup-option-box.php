                ...
                <div class="tab-content" id="myTabContent">
                    ...
                    <div class="tab-pane fade pt-3 show active" id="gen-setting-tab-pane" role="tabpanel" aria-labelledby="gen-setting-tab" tabindex="0">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" id="hqmm-title" class="form-control" value="HỘP QUÀ MAY MẮN ONLINE" placeholder="HỘP QUÀ MAY MẮN ONLINE">
                        </div>

                        <!-- Danh sách phần thưởng -->
                        <div class="mb-3">
                            <label for="section-list" class="form-label fw-bold"><?php esc_html_e('Danh sách phần thưởng (mỗi dòng 1 phần thưởng)', 'wp-spin-wheel'); ?></label>
                            <textarea id="section-list" class="form-control" rows="6" placeholder="Mỗi dòng tương ứng với một phần thưởng">100k
Ốp lưng iphone
50k
Chúc bạn may mắn
200k
...
Hộp đựng cơm</textarea>
                            <div class="form-text"><?php esc_html_e('Các hộp quà khi mở sẽ bốc ngẫu nhiên một trong các phần quà trên.', 'wp-spin-wheel'); ?></div>
                        </div>

                        <!-- Số lượt mở -->
                        <div class="mb-3">
                            <label for="luotchoi" class="form-label fw-bold"><?php esc_html_e('Số lượt mở tối đa', 'wp-spin-wheel'); ?></label>
                            <select class="form-select" id="luotchoi">
                                <?php for ($i = 1; $i <= 12; $i++) : ?>
                                    <option value="<?php echo esc_attr($i); ?>" <?php selected($i, 3); ?>><?php echo sprintf(esc_html__('%d lượt mở', 'wp-spin-wheel'), $i); ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        ...
                        <!-- ♪ Âm thanh khi mở -->
                        <div class="mb-3">
                            <label class="form-label fw-bold"><?php esc_html_e('Âm thanh khi mở hộp', 'wp-spin-wheel'); ?></label>
                            <div class="input-group mb-1">
                                ...
                                <select class="form-select" id="sound">
                                    ...
                                    <option value="winner" selected><?php esc_html_e('Winner (Mặc định)', 'wp-spin-wheel'); ?></option>
                                    <option value="congratulation"><?php esc_html_e('Chúc mừng', 'wp-spin-wheel'); ?></option>
                                    ...
                                    <option value="phaohoano"><?php esc_html_e('Pháo hoa nổ', 'wp-spin-wheel'); ?></option>
                                    <?php if (! empty($_audios_open)) : ?>
                                        <optgroup label="── Thư viện nhạc ──">
                                            <?php foreach ($_audios_open as $_ao) : ?>
                                                <option value="<?php echo esc_attr($_ao['id'] ?? ''); ?>" data-url="<?php echo esc_url($_ao['config']['file'] ?? ''); ?>"><?php echo esc_html($_ao['name'] ?? ''); ?></option>
                                            <?php endforeach; ?>
                                        </optgroup>
                                    <?php endif; ?>
                                </select>
                                <button type="button" class="btn btn-outline-secondary" id="btn-sound-play" title="Nghe thử">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
                                </button>
                            </div>
                            <div class="input-group input-group-sm">
                                <span class="input-group-text"><a class="text-decoration-none" target="_blank" href="https://nhactik.com">♪ nhactik.com</a></span>
                                <input type="text" class="form-control" id="sound_file" placeholder="File ID (vd: 12345)">
                                <button type="button" class="btn btn-outline-secondary" id="btn-sound-file-play" title="Nghe thử nhactik">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold"><?php esc_html_e('Âm thanh khi hết lượt', 'wp-spin-wheel'); ?></label>
                            <div class="input-group mb-1">
                                <span class="input-group-text">♪ Khi hết</span>
                                <select class="form-select" id="noti_sound">
                                    <option value="0"><?php esc_html_e('Tắt tiếng', 'wp-spin-wheel'); ?></option>
                                    <option value="random"><?php esc_html_e('Ngẫu nhiên', 'wp-spin-wheel'); ?></option>
                                    <option value="concainit" selected><?php esc_html_e('Còn cái nịt', 'wp-spin-wheel'); ?></option>
                                    <option value="complete"><?php esc_html_e('Hoàn thành', 'wp-spin-wheel'); ?></option>
                                    ...
                                    <?php if (! empty($_audios_end)) : ?>
                                        <optgroup label="── Thư viện nhạc ──">
                                            <?php foreach ($_audios_end as $_ae) : ?>
                                                <option value="<?php echo esc_attr($_ae['id'] ?? ''); ?>" data-url="<?php echo esc_url($_ae['config']['file'] ?? ''); ?>"><?php echo esc_html($_ae['name'] ?? ''); ?></option>
                                            <?php endforeach; ?>
                                        </optgroup>
                                    <?php endif; ?>
                                </select>
                                <button type="button" class="btn btn-outline-secondary" id="btn-noti_sound-play" title="Nghe thử">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
                                </button>
                            </div>
                            <div class="input-group input-group-sm">
                                ...
                                <button type="button" class="btn btn-outline-secondary" id="btn-noti_sound-file-play" title="Nghe thử nhactik">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg>
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            <label for="popup_title" class="form-label fw-bold"><?php esc_html_e('Tiêu đề popup trúng thưởng', 'wp-spin-wheel'); ?></label>
                            <input type="text" class="form-control" id="popup_title" value="Hộp quà có" placeholder="Hộp quà có">
                        </div>

                        <div class="form-check mb-2">
                            <input class="form-check-input" type="checkbox" id="confetti" checked>
                            <label class="form-check-label fw-semibold" for="confetti"><?php esc_html_e('Bắn hoa giấy khi mở trúng hộp quà', 'wp-spin-wheel'); ?></label>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade pt-3" id="appearance-tab-pane" role="tabpanel" aria-labelledby="appearance-tab" tabindex="0">
                        ...
                        <fieldset class="border border-2 rounded-3 p-3 mb-3">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-bold"><?php esc_html_e('Chọn kiểu mẫu:', 'wp-spin-wheel'); ?></label>
                                <input type="hidden" id="template" value="tpl-jib">
                                <div class="dropdown" id="boxTemplateDropdown">
                                    <button class="btn btn-outline-secondary w-100 d-flex align-items-center justify-content-between dropdown-toggle" id="btn-dropdown-select-tpl" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="fs-5">📦</span>
                                            <span class="item-title fw-bold">Ball in box (Mặc định)</span>
                                        </div>
                                    </button>
                                    <div class="dropdown-menu w-100 shadow-lg" id="btn-select-tpl" style="max-height: 280px; overflow-y: auto;">
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-jib" data-title="Ball in box (Mặc định)"><span class="me-2">📦</span> Ball in box (Mặc định)</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-default" data-title="Hộp quà truyền thống"><span class="me-2">🎁</span> Hộp quà truyền thống</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-christmas" data-title="Giáng sinh Christmas"><span class="me-2">🎄</span> Giáng sinh Christmas</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-money-bag" data-title="Túi tiền tài lộc"><span class="me-2">💰</span> Túi tiền tài lộc</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-blind-bag" data-title="Túi mù bí ẩn"><span class="me-2">🛍️</span> Túi mù bí ẩn</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-money" data-title="Bao lì xì Tết"><span class="me-2">🧧</span> Bao lì xì Tết</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-egg" data-title="Đập trứng vàng"><span class="me-2">🥚</span> Đập trứng vàng</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-jar" data-title="Đập lu đất"><span class="me-2">🏺</span> Đập lu đất</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-rat" data-title="Đập chuột may mắn"><span class="me-2">🐭</span> Đập chuột may mắn</div>
                                        <div class="dropdown-item py-2 cursor-pointer" data-content="tpl-ghost" data-title="Halloween Diệt ma"><span class="me-2">👻</span> Halloween Diệt ma</div>
                                    </div>
                                </div>
                            </div>
                        </fieldset>
                        ...
                        <!-- Màu sắc & Nút -->
                        <fieldset class="border border-2 rounded-3 p-3 mb-3">
                            <legend class="float-none w-auto px-2 fs-6 fw-bold"><?php esc_html_e('Màu sắc giao diện', 'wp-spin-wheel'); ?></legend>
                            <div class="row g-2 mb-3">
                                <div class="col-6">
                                    <label class="form-label small fw-bold text-muted"><?php esc_html_e('Màu nền', 'wp-spin-wheel'); ?></label>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text"><?php esc_html_e('Nền', 'wp-spin-wheel'); ?></span>
                                        <input type="color" class="form-control form-control-color" id="bg_color" value="#dc3545">
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small fw-bold text-muted"><?php esc_html_e('Màu chữ', 'wp-spin-wheel'); ?></label>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text"><?php esc_html_e('Chữ', 'wp-spin-wheel'); ?></span>
                                        <input type="color" class="form-control form-control-color" id="color" value="#ffffff">
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small fw-bold text-muted"><?php esc_html_e('Màu nút bấm', 'wp-spin-wheel'); ?></label>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text"><?php esc_html_e('Nút', 'wp-spin-wheel'); ?></span>
                                        <input type="color" class="form-control form-control-color" id="btn_bg_color" value="#dc3545">
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small fw-bold text-muted"><?php esc_html_e('Màu chữ nút', 'wp-spin-wheel'); ?></label>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text"><?php esc_html_e('Chữ nút', 'wp-spin-wheel'); ?></span>
                                        <input type="color" class="form-control form-control-color" id="btn_color" value="#ffffff">
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Ảnh nền -->
                            <div class="mb-1 small fw-semibold text-muted"><?php esc_html_e('Ảnh nền', 'wp-spin-wheel'); ?></div>
                            ...
                            <div class="input-group input-group-sm mb-2">
                                ...
                                <input type="url" class="form-control" id="bg_img" placeholder="https://example.com/bg.jpg" value="">
                                <button class="btn btn-outline-secondary" id="btn-apply-box-img" type="button"><?php esc_html_e('Áp dụng', 'wp-spin-wheel'); ?></button>
                            </div>
                            <div id="box-bgr-preview-wrap" style="display:none;" class="mb-2 d-flex align-items-center gap-2">
                                <img id="box-bgr-preview" src="https://placehold.co/100x100" alt="" width="60" height="40" style="object-fit:cover;border-radius:4px;border:1px solid #ccc;flex-shrink:0;">
                                <button class="btn btn-outline-danger btn-sm py-0 px-2" id="btn-clear-box-img" type="button">✕ <?php esc_html_e('Xoá ảnh', 'wp-spin-wheel'); ?></button>
                            </div>

                            <hr class="my-2">

                            <!-- Gradient -->
                            <div class="mb-1 small fw-semibold text-muted"><?php esc_html_e('Gradient', 'wp-spin-wheel'); ?></div>
                            ...
                            <div class="d-flex gap-2 align-items-start mb-2">
                                <div id="box-gradient-preview-box" style="width:44px;height:44px;border-radius:6px;border:1px solid #ccc;flex-shrink:0;transition:.2s;background:conic-gradient(from 90deg,#df3000,#feb81a,#df3000);"></div>
                                <textarea class="form-control form-control-sm" id="bg_gradient" rows="3" placeholder="Nhập CSS gradient (vd: conic-gradient(...) hoặc linear-gradient(...))"></textarea>
                            </div>
                    ...
                    <div class="tab-pane fade pt-3" id="media-tab-pane" role="tabpanel" aria-labelledby="media-tab" tabindex="0">
                        <div id="box-uploaded-list" class="mb-3">
                            <ul class="nav nav-tabs" id="boxMediaTabs" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link active" id="box-background-tab" data-bs-toggle="tab" data-bs-target="#box-background-pane" type="button" role="tab" aria-controls="box-background-pane" aria-selected="true">
                                ...
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link" id="box-gradient-tab" data-bs-toggle="tab" data-bs-target="#box-gradient-pane" type="button" role="tab" aria-controls="box-gradient-pane" aria-selected="false">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="me-1"><circle cx="12" cy="12" r="10"></circle><path d="M12 2a10 10 0 0 1 0 20z"></path></svg>
                            ...
                            <div class="tab-content" id="boxMediaTabsContent">
                                <!-- Subtab 1: Ảnh nền -->
                                <div class="tab-pane fade show active" id="box-background-pane" role="tabpanel" aria-labelledby="box-background-tab" tabindex="0">
                                            ...
                                            <tbody id="mediaBoxBackgroundBody">
                                                ...
                                                ?>
                                                        <tr>
                                                            ...
                                                            <td class="small fw-semibold"><?php echo esc_html($_bgr_name); ?></td>
                                                            <td><img src="<?php echo esc_url($_bgr_file_url); ?>" width="50" height="50" class="border border-1 rounded-1" style="object-fit:cover;"></td>
                                                            <td>
                                                                <button class="btn btn-sm btn-secondary sw-box-media-apply" data-type="bgr" data-url="<?php echo esc_url($_bgr_file_url); ?>"><?php esc_html_e('Nền', 'wp-spin-wheel'); ?></button>
                                                            </td>
                                                ...
                                                <?php else : ?>
                                                    <tr>
                                                        <td colspan="5" class="text-center text-muted small"><?php esc_html_e('Không có ảnh nền', 'wp-spin-wheel'); ?></td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        </table>
                                ...
                                <!-- Subtab 2: Gradient -->
                                <div class="tab-pane fade" id="box-gradient-pane" role="tabpanel" aria-labelledby="box-gradient-tab" tabindex="0">
                                    <div class="mt-3" style="max-height: 350px; overflow-y: auto; overflow-x: hidden;">
                                            ...
                                            <tbody id="mediaBoxGradientBody">
                                                ...
                                                ?>
                                                        <tr>
                                                            ...
                                                            <td class="small fw-semibold"><?php echo esc_html($_grd_name); ?></td>
                                                            <td>
                                                                <div class="sw-gradient-preview" data-gradient="<?php echo esc_attr($_grd_item['value']); ?>" style="width:50px;height:50px;border:1px solid #ccc;border-radius:4px;background:<?php echo esc_attr($_grd_item['value']); ?>;"></div>
                                                            </td>
                                                            <td>
                                                                <button class="btn btn-sm btn-secondary sw-box-media-apply" data-type="grd" data-gradient="<?php echo esc_attr($_grd_item['value']); ?>"><?php esc_html_e('Nền', 'wp-spin-wheel'); ?></button>
                                                            </td>
                                                            <td class="text-muted">∗</td>
                                                        </tr>
                                                    <?php endforeach; ?>
                                                <?php else : ?>
                                                    <tr>
                                                        <td colspan="5" class="text-center text-muted small"><?php esc_html_e('Không có gradient', 'wp-spin-wheel'); ?></td>
                                                    </tr>
                                                <?php endif; ?>
                                            </tbody>
                                        </table>
            ...
            <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-outline-secondary btn-sm" id="btn-reset-box"><?php esc_html_e('Đặt lại', 'wp-spin-wheel'); ?></button>
                <div class="d-flex gap-2">
                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal"><?php esc_html_e('Đóng', 'wp-spin-wheel'); ?></button>
                    <button type="button" class="btn btn-primary btn-sm px-3" id="btn_box_setting"><?php esc_html_e('Lưu lại', 'wp-spin-wheel'); ?></button>
                </div>
            </div>
        </div>
    </div>
